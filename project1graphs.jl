...
# ╔═╡ a4292446-ad29-4899-b78e-eef056242d31
using Symbolics, Latexify

# ╔═╡ 1a21af0d-3619-4665-8a5b-4f40dd5331a0
begin
...
# ╔═╡ 8aca4032-b53b-4b84-836e-2f21cc3803e8
θ̈ = D(D(θ))

# ╔═╡ a4a7cb5b-933f-4a4c-95f9-365162e6b913
ode = Equation(θ̈, -(g/L)*sin(θ))

# ╔═╡ 0fee38f9-e1c0-410a-ab18-8d0a00f3d92c


# ╔═╡ 72e9597c-bdb5-4b77-9d8f-2728f2996c62
begin 
	u0_slow = [0.2, 0.5]            
	tspan_slow = (0.0, 10.0)
		
	prob_slow = ODEProblem(pend!, u0_slow, tspan_slow)
	sol_slow = solve(prob_slow, Tsit5())
end

# ╔═╡ 9d5fbf4a-f0b0-4400-857e-4b22e5c97b91


# ╔═╡ fe2962b3-af0a-409a-af90-d3bb45f3e622


# ╔═╡ 5550622a-1fc6-4ac5-809b-f1167be0e535
begin 
	u0_fast = [0.0, 3.0]             
	tspan_fast = (0.0, 8.0)
		
	prob_fast = ODEProblem(pend!, u0_fast, tspan_fast)
	sol_fast = solve(prob_fast, Tsit5())
end

# ╔═╡ cf8b74e9-f96e-4f5a-9fd2-9145a7f03cb0
begin
	plot(sol_slow[1,:], vars=(0,1), xlabel="t", ylabel="θ", label="slow motion")
	plot!(sol_fast[1,:], vars=(0,1), title="Motion", label = "fast motion")
end

# ╔═╡ bcb10331-9660-4d02-8af7-c96ee77b0645


# ╔═╡ 38525ca2-a360-4669-839c-947afaa5c328


# ╔═╡ 831fc47f-8cbc-4bfc-abd9-b1abd17cfcea


# ╔═╡ ba80931a-dbeb-4ee7-b4e1-45d36240ea71


# ╔═╡ 2ce5e993-56f4-48e6-a832-4d5c7dbd9710


# ╔═╡ 1d9c351a-9158-4094-8ab1-a35349f7f9d0
function anim_pend(sol, name)
	anim = @animate for k in 1:3:length(sol.t)
		θ = sol.u[k][1]
		x = L_val * sin(θ)
		y = -L_val * cos(θ)
			plot(xlim=(-1.2,1.2), ylim=(-1.2,0.3), aspect_ratio=:equal,
		    legend=false, framestyle=:none, title=name)
		    plot!([0,x], [0,y], lw=3, color=:blue)
		    scatter!([x], [y], markersize=8, color=:red)
		    end
		gif(anim, name * ".gif", fps=30)
		end

# ╔═╡ 926ef1b1-002f-4860-b3fa-81a5d1081b8f
anim_pend(sol_slow, "slow_motion")


# ╔═╡ 474af808-1a5e-4a26-b489-d98d56f9b68f
anim_pend(sol_fast, "fast_motion")
...
manifest_format = "2.0"
project_hash = "8102374553aaa0e8c373d09150b4ad27ad990377"
...
git-tree-sha1 = "6ccf0c6b8fea7079940d1eaa1b8cb94270dba0aa"
